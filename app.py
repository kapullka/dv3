# app.py — Employee management with admin controls, fixed panel, editable plans
...
with col_right:
    ...
    password_emp = st.text_input("Admin password", type="password", key="pw_emp")
    ...
    # Admin password for adding new month
    password_new_month = st.text_input("Admin password for new month", type="password", key="pw_new_month")
    if st.button("➕ Add New Month"):
        if password_new_month == ADMIN_PASSWORD:
            ...
            st.error("Invalid admin password! Cannot add new month.")

# -------------------- Ensure selected month exists and normalized --------------------
...
if len(md.get("weeks", [])) != len(expected_weeks):
    ...
    for idx, week_dates in enumerate(expected_weeks):
        ...
        for emp in md.get("employees", []):
            if old_w and emp in old_w.get("daily_profits", {}):
...
# -------------------- Main area: weeks in one row with Employee Panel --------------------
